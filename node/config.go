...
import (
	...
	"github.com/elastos/Elastos.ELA.SideChain.ECO/accounts/external"
	...
	"github.com/elastos/Elastos.ELA.SideChain.ECO/accounts/usbwallet"
...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of eco is "eco". If no
...
// These resources are resolved differently for "eco" instances.
...
func (c *Config) ResolvePath(path string) string {
	...
	// by eco 1.4 are used if they exist.
	if warn, isOld := isOldGethResource[path]; isOld {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldGethResourceWarning, "Using deprecated resource file %s, please move this file to the 'eco' subdirectory of datadir.", oldpath)
			}
			return oldpath
		}
	}
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "Elastos.ELA.SideChain.ECO-keystore")
		ephemeral = keydir
	}
